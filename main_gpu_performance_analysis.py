...
def init_performance_csv(csv_filename="performance_log.csv"):
    """Initialize the CSV file for performance logging."""
    fieldnames = [
        'iteration', 'timestamp', 'total_processing_time_seconds',
        'ram_total_gb', 'ram_used_after_loading_gb', 'ram_free_after_loading_gb', 'ram_usage_percent',
        'gpu_total_gb', 'gpu_used_after_loading_gb', 'gpu_free_after_loading_gb', 'gpu_usage_percent'
    ]
...
def log_performance_data(csv_filename, iteration, total_time, memory_after_loading, fieldnames):
    ...
    row_data = {
        ...
        'ram_total_gb': memory_after_loading['ram_total'],
        'ram_used_after_loading_gb': memory_after_loading['ram_used'],
        'ram_free_after_loading_gb': memory_after_loading['ram_free'],
        'ram_usage_percent': (memory_after_loading['ram_used'] / memory_after_loading['ram_total']) * 100,
        'gpu_total_gb': memory_after_loading['gpu_total'],
        'gpu_used_after_loading_gb': memory_after_loading['gpu_used'],
        'gpu_free_after_loading_gb': memory_after_loading['gpu_free'],
        'gpu_usage_percent': (memory_after_loading['gpu_used'] / memory_after_loading['gpu_total']) * 100 if memory_after_loading['gpu_total'] > 0 else 0
    }
    ...
    print(f"Performance data logged to {csv_filename} (iteration {iteration})")

def cleanup_checkpoints(checkpoint_dir='checkpoints'):
    ...
    if os.path.exists(checkpoint_dir):
        try:
            ...
            print(f"Checkpoint directory '{checkpoint_dir}' cleaned up successfully.")
        except Exception as e:
            print(f"Warning: Could not clean up checkpoint directory '{checkpoint_dir}': {e}")
    else:
        print(f"Checkpoint directory '{checkpoint_dir}' does not exist, nothing to clean.")
...
class SARSuperOptimizedFocuser:
    ...
    def process_dataset(self, data_dir='.'):
        ...
        # Initialize variable to store memory info after loading
        memory_after_loading = None
        ...
        with h5py.File(self.output_file, h5_mode) as h5f:
            ...
            for batch_idx in range(num_total_dem_batches):
                ...
                # Capture memory usage after loading raw data chunk (for CSV logging)
                memory_after_loading = get_memory_info()
                
                # Store this as the instance variable so it can be returned
                self.memory_after_loading = memory_after_loading
            ...
            # Return processing time and memory info for logging
            total_processing_time = time.time() - total_start_time_for_this_run
            return total_processing_time, getattr(self, 'memory_after_loading', None)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='SAR Back-Projection Processor with Performance Logging')
    parser.add_argument('--n', type=int, default=1, help='Number of iterations to run (default: 1)')
    parser.add_argument('--csv', type=str, default='performance_log.csv', help='CSV file to log performance data (default: performance_log.csv)')
    ...
    print(" Performance Logging Edition")
    print("="*80)
    
    print(f"\nConfiguration:")
    print(f" Number of iterations: {args.n}")
    print(f" CSV log file: {args.csv}")
    ...
    fieldnames = init_performance_csv(args.csv)
    print(f"\nPerformance logging initialized: {args.csv}")
    
    # Run multiple iterations
    for iteration in range(1, args.n + 1):
        print(f"\n{'='*80}")
        print(f" ITERATION {iteration}/{args.n}")
        print(f"{'='*80}")
        
        focuser = SARSuperOptimizedFocuser()
        
        try:
            # Process dataset and get performance data
            total_time, memory_after_loading = focuser.process_dataset(args.data_dir)
            
            # Log performance data to CSV
            if memory_after_loading is not None:
                log_performance_data(args.csv, iteration, total_time, memory_after_loading, fieldnames)
            else:
                print(f"Warning: Memory data not captured for iteration {iteration}")
            
            # Only generate preview and export on last iteration to save time
            if iteration == args.n:
                print("\nGenerating final preview and exporting formats...")
                focuser.generate_preview()
                focuser.export_to_formats()
            
            print(f"\nIteration {iteration}/{args.n} completed successfully!")
            ...
            print(f"\nProcessing interrupted by user at iteration {iteration}.")
            break
        except Exception as e:
            print(f"\nError during processing at iteration {iteration}: {e}")
            import traceback
            traceback.print_exc()
    ...
    print(f" Performance data saved to: {args.csv}")
    print(f"{'='*80}")